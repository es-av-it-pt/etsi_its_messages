...
namespace displays
{
    ...
    > & denm_variant)
  {
    ...
    std::string cause_code = "Not present";
    std::string sub_cause_code = "Not present";
    double heading_deg = 0 * M_PI / 180.0;
    ...
    if (const auto * r1 = std::get_if<etsi_its_denm_msgs::msg::DENM>(&denm_variant)) {
      ...
      if (r1->denm.situation_is_present) {
        cause_code = etsi_its_denm_msgs::access::getCauseCodeType(*r1);
        sub_cause_code = etsi_its_denm_msgs::access::getSubCauseCodeType(*r1);
      }
      if (r1->denm.location_is_present && etsi_its_denm_msgs::access::getIsHeadingPresent(*r1)) {
        heading_deg = etsi_its_denm_msgs::access::getHeading(*r1);
      }
      ...
      is_ts = true;
      // TODO: implement DENM TS access functions
      /*p = etsi_its_denm_ts_msgs::access::getUTMPosition(*r2, zone, northp);
      // Use TS access helper to compute unix nanoseconds from reference / delta if available.
      ...
      if (r2->denm.situation_is_present) {
        cause_code = etsi_its_denm_ts_msgs::access::getCauseCodeType(*r2);
        sub_cause_code = etsi_its_denm_ts_msgs::access::getSubCauseCodeType(*r2);
      }
      if (r2->denm.location_is_present && etsi_its_denm_ts_msgs::access::getIsHeadingPresent(*r2)) {
        heading_deg = etsi_its_denm_ts_msgs::access::getHeading(*r2);
      }
    ...
    cause_code_type = cause_code;
    sub_cause_code_type = sub_cause_code;
    double heading = (90.0 - heading_deg) * M_PI / 180.0; // 0.0° equals WGS84 North, 90.0° equals WGS84 East, 180.0° equals WGS84 South and 270.0° equals WGS84 West
